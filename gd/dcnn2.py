...
class tDataset(Dataset):
    def __init__(self,image, label):
        ...
        images=[]
        labels=[]
        imagepath=data_path+'/'+image
        imagelist=os.listdir(imagepath)
        for j in imagelist:
            images.append(os.path.join(imagepath,j))
            labels.append(labeltoint(label))
        self.images=np.array(images)
        self.labels=torch.LongTensor(np.array(labels))
        self.transforms=transforms.Compose([
            transforms.ToTensor(),
...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.conv=nn.Sequential(
            ...
            # 32*63
            nn.Conv2d(32, 48, kernel_size=5,stride=3),
            nn.BatchNorm2d(48),
            nn.ReLU(),
            nn.MaxPool2d(2),
            nn.Dropout2d(Dropoutrate),
...
def modeltest(model,images,labels):
    ...
    # for j in range(405, 406):
        testset = tDataset(images[j], labels[j])
        test_loader = DataLoader(dataset=testset,
                                 batch_size=batch_size,
                                 shuffle=False,
                                 num_workers=2
...
def getfold(i):
    ...
    testdata=[]
    for j in list:
        trainsessionpath = sessionpath   + 'Session' + str(j + 1) + '.txt'
        traindatalist = open(trainsessionpath)
        for data in traindatalist:
            data = data.strip('\n')
            traindata.append(data.split())
    testsessionpath = sessionpath + 'Session' + str(i + 1) + '.txt'
...
def plot_confusion_matrix(cm, classes, normalize=False, title='Confusion matrix', cmap=plt.cm.Blues):
    ...
    plt.savefig('../figure/iemocap_dcnn2')
# 更新混淆矩阵
...
def finaltest(model):
    ...
    for i in range(foldnum):
        ...
        for data in testdatalist:
            data = data.strip('\n')
            testdata.append(data.split())
        images = np.array(testdata)[:,0]
        labels = np.array(testdata)[:,1]
        for j in range(len(images) ):
        # for j in range(405, 406):
            testset = tDataset(images[j], labels[j])
            test_loader = DataLoader(dataset=testset,
...
def imagenum(i):
    sessionfilepath = '../iemocap/spectrogram/'
    sessionpath = sessionfilepath + 'Session' + str(i + 1)
    num=[0]
    count=0
...
# nohup python dcnn2.py > dcnn2.out &
